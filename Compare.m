function [eps tan_loss] = Compare(S21c_filename, S21r_filename, coeffs_filename, doPlot)
	...
	[f_eps_1 f_eps_2 f_tloss_1 f_tloss_2] = ReadCoeffs(coeffs_filename);
	...
	q_factors = zeros(2,2);
%	qFactors(1, 1) = QFactorFinder(S21_ref, resonance_freqs(1,1), f_range_ref, .5);
%	qFactors(1, 2) = QFactorFinder(S21_ref, resonance_freqs(1,2), f_range_ref, .5);
%
%	qFactors(2, 1) = QFactorFinder(S21_comp, resonance_freqs(2,1), f_range_ref, .5);
%	qFactors(2, 2) = QFactorFinder(S21_comp, resonance_freqs(2,2), f_range_ref, .5);
